...
# 监控%19 pane的守护进程启动脚本
# 使用opencode-watch监控tmux pane %19
...
INTERVAL=8      # 8秒检查一次，避免过于频繁
STUCK_SEC=40    # 40秒无变化判定为卡住（5次无变化）
MAX_RETRY=5     # 最大重试次数

# 日志配置
LOG_DIR="/var/log/opencode-watch"
LOG_FILE="${LOG_DIR}/monitor_19.log"
PID_FILE="${LOG_DIR}/monitor_19.pid"
...
echo "$(date): 守护进程已启动，PM2进程名称: opencode-watch-19" >> "$LOG_FILE"
echo "使用以下命令查看状态:"
echo "  pm2 status opencode-watch-19"
echo "  pm2 logs opencode-watch-19"
echo "  pm2 monit"